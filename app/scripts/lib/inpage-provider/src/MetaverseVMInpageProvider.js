...
module.exports = class MetaverseVMInpageProvider extends SafeEventEmitter {
  ...
  constructor (
    ...
    this._metaversevm = this._getExperimentalApi()
    ...
    pump(
      ...
      mux,
      connectionStream,
      this._handleDisconnect.bind(this, 'MetaverseVM'),
    )

    // subscribe to MetaverseVM public config (one-way)
    this._publicConfigStore = new ObservableStore({ storageKey: 'MetaverseVM-Config' })
  ...
  /**
   * Returns whether the inpage provider is connected to MetaverseVM.
   */
  isConnected () {
  ...
  /**
   * Submits an RPC request to MetaverseVM for the given method, with the given params.
  ...
  _handleAccountsChanged (accounts, isEthAccounts = false, isInternal = false) {
    ...
    if (!Array.isArray(accounts)) {
      log.error(
        'MetaverseVM: Received non-array accounts parameter. Please report this bug.',
        accounts,
      )
      _accounts = []
    }
    ...
    if (!dequal(this._state.accounts, _accounts)) {
      ...
      if (isEthAccounts && this._state.accounts !== undefined && !isInternal) {
        log.error(
          `MetaverseVM: 'eth_accounts' unexpectedly updated accounts. Please report this bug.`,
          _accounts,
        )
      }
  ...
  /**
   * Constructor helper.
   * Gets experimental metaversevm API as Proxy, so that we can warn consumers
    ...
    return new Proxy(
      {
        ...
        /**
         * Determines if MetaverseVM is unlocked by the user.
         *
         * @returns {Promise<boolean>} - Promise resolving to true if MetaverseVM is currently unlocked
         */
        isUnlocked: async () => {
  ...
  /**
   * DEPRECATED.
   * Sends an RPC request to MetaverseVM.
